...
am4core.ready(function() {
	...
	var currentIndex;
	var currentWilaya = "Algérie (Total)";
	...
	// pour ignoré le valeur egale a 0
	...
	//  zoom-out dans click exterieur au map
...
function animateBullet(circle) {
	...
	// switch entre affichage Bulle ou bien par Zone
	var switchbuLleZone = mapChart.createChild(am4core.SwitchButton);
	switchbuLleZone.align = "left"
	switchbuLleZone.y = 45;
	switchbuLleZone.leftLabel.text = "Affichage Bulle";
	switchbuLleZone.leftLabel.fill = am4core.color("#ffffff");
	switchbuLleZone.rightLabel.fill = am4core.color("#ffffff");
	switchbuLleZone.rightLabel.text = "Coloriage par Zone";
	switchbuLleZone.rightLabel.interactionsEnabled = false;
	switchbuLleZone.verticalCenter = "top";


	switchbuLleZone.events.on("toggled", function() {
		if (switchbuLleZone.isActive) {
			bubbleSeries.hide(0);
			perCapita = true;
...
var langSwitch = mapChart.createChild(am4core.SwitchButton);
	...
	// Wilaya name and buttons container
	...
	// Affichage Nom wilaya et la date
	var wilaya = nameAndButtonsContainer.createChild(am4core.Label);
	wilaya.fontSize = "1.1em";
	wilaya.fill = am4core.color("#ffffff");
	wilaya.valign = "middle";

	// Buttons (confirmed/deaths)
	...
	// Arret animation au glissage du slider
	...
	// personaliser le bouton du Slider GRIP
  ...
  dateAxis.renderer.labels.template.adapter.add("fillOpacity", function(fillOpacity, target){
	...
	seriesTypeSwitch.leftLabel.text = "par mois";
	seriesTypeSwitch.rightLabel.text = "par jour"
	...
	// select a country
	function selectWilaya(mapPolygon) {
		resetHover();
		...
		var wilayaiNdex = countryIndexMap[mapPolygon.dataItem.id];
		currentWilaya = mapPolygon.dataItem.dataContext.name;
	...
	function setCountryData(wilayaiNdex) {
		...
		for (var i = 0; i < lineChart.data.length; i++) {
			...
			var countryData = di[wilayaiNdex];
	...
	function updateCountryName() {
		wilaya.text = currentWilaya + ", " + mapChart.dateFormatter.format(currentDate, "MMM dd, yyyy");
	}
	...
	function handleImageHit(event) {
		selectWilaya(polygonSeries.getPolygonById(event.target.dataItem.id));
	}

	function handleCountryHit(event) {
		selectWilaya(event.target);
	}
	...
	function populateCountries(list) {
		...
		for (var i = 0; i < list.length; i++) {
			...
			var tr = $("<tr>").addClass("area").data("areaid", area.id).appendTo(table).on("click", function() {
				selectWilaya(polygonSeries.getPolygonById($(this).data("areaid")));
			}).hover(function() {
...
function idToName(id) {
	...
	return am4geodata_data_wilaya_ar[id] ? am4geodata_data_wilaya_ar[id] : id;
}
